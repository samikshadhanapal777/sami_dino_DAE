...
// ---- CONFIG: auto-detect origin so it works on any port ----
...
// =============================================
//   SIGN OUT
//   Clears PHP session + localStorage fallback
// =============================================
async function signOut() {
    ...
    try {
        ...
        await res.json(); // wait for full response before redirecting
    } catch(e) { /* MAMP not running, that's fine */ }
    localStorage.removeItem('sp_user');
    localStorage.setItem('sp_first_visit', '0');
    window.location.replace('login.html'); // replace so back button doesn't go to index
}
